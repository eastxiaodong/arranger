...
export function createTools(context: vscode.ExtensionContext, options: ToolFactoryOptions): Tool[] {
  ...
  const pickApproverByRole = (role?: string) => {
    ...
    const candidates = options.services.agent.getAgentsByCapability(normalized);
  ...
  return [
    ...
    },
    {
      ...
      handler: async (input: any) => {
        ...
        // Approval service removed - approval workflow no longer supported
        options.services.notification.sendNotification({
          session_id: sessionId,
          level: 'info',
          title: '任务已提交',
          message: `${agentInfo().displayName} 已提交任务 ${taskId} 给 ${approverId}`
        });
