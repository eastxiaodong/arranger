...
export class MCPService {
  ...
  async callTool(serverName: string | undefined, toolName: string, payload: Record<string, any>) {
    const server = this.resolveServer(serverName);
    const connection = await this.getConnection(server);
    return this.invokeTool(server, connection, toolName, payload);
  }
  ...
  private async invokeTool(server: MCPServer, connection: MCPServerConnection, toolName: string, payload: Record<string, any>) {
