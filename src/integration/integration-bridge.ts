...
import type { WorkflowRuntimeEvent } from '../workflow';
import type { ProofRecord } from '../types';
import type { SentinelEventPayload } from '../orchestration/orchestration-types';
...
export class IntegrationBridge implements vscode.Disposable {
  ...
  private readonly workflowHandler = (event: WorkflowRuntimeEvent) => {
    void this.dispatch('workflow_event', event);
  };
  private readonly sentinelHandler = (event: SentinelEventPayload) => {
    void this.dispatch('sentinel_event', event);
  };
  ...
  private subscribe() {
    ...
    this.events.on('workflow_event', this.workflowHandler);
    this.events.on('sentinel_event', this.sentinelHandler);
  ...
  private unsubscribe() {
    ...
    this.events.off('workflow_event', this.workflowHandler);
    this.events.off('sentinel_event', this.sentinelHandler);
